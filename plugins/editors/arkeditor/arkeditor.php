...
class plgEditorArkEditor extends JPlugin {
	...
	public function onDisplay($name, $content, $width, $height, $col, $row, $buttons = true, $id = null, $asset = null, $author = null)
	{
		...
		if($this->inlineMode == ArkInlineMode::INLINE)
        {
			...
			$config->textalign						= 	$params->def( 'textalign',0);
			$config->entities						= 	$params->def( 'entities',0);
  			$formatsource							= 	$params->def( 'formatsource',1);
			$config->magicline_enabled				= 	(int) $params->get( 'magicline_enabled',1);	
			$config->stylesheets					=	$params->def('stylesheets',array('model','bootstrap','frontdediting'));	
			...
			$config->autoDisableInline				=   (int) $params->def( 'auto_disable_inline',0);
			...
			{
			 case 0:
				 $config->defaultLanguage = $config->language; // User selection
				 $config->direction = $params->get( 'direction','ltr');
				 break;
			 case 1:
				$config->defaultLanguage = 	""; // Joomla Default
				$config->language = substr( $language->getTag(), 0, strpos( $language->getTag(), '-' ) ); //Access Joomla's global configuation and get the language setting from there
				break;
			 case 2:
				$config->defaultLanguage = 	""; // Browser default
				$config->language = "";
				break; 
			}
		...
		$textalign						= 	$params->def( 'textalign',0);
		$entities						= 	$params->def( 'entities',0);
		$formatsource					= 	$params->def( 'formatsource',1);
		$magicline_enabled				= 	$params->def( 'magicline_enabled',1);
		$toolbars						=	$this->_decode($params->get('toolbars'));
		$excludes 						= 	$params->def('stylesheets',array('model','bootstrap','frontdediting'));
		...
		{
		 case 0:
			 $defaultLanguage = 	$lang; // User selection
			 $direction = $params->get( 'direction','ltr');
			 break;
		 case 1:
			$defaultLanguage = 	""; // Joomla Default
			$lang = substr( $language->getTag(), 0, strpos( $language->getTag(), '-' ) ); //Access Joomla's global configuation and get the language setting from there
			break;
		 case 2:
			$defaultLanguage = 	""; // Browser default
			$lang = "";
			break; 
		}
		...
		$document->addScriptDeclaration("
			...
			{
				//addCustom CSS
				CKEDITOR.addCss( 'body { background: ". $bgcolor . " none;".$textalign ."}' );
		...
		$document->addScriptDeclaration("
			CKEDITOR.on( 'instanceCreated', function( evt ) {
				...
				evt.editor.on( 'configLoaded', function() {
					...
					this.config.stylesheetParser_skipSelectors = /(^body\.|cke_|__|sbox|^input|^textarea|^button|^select|^form|^fieldset|^\.modal-backdrop|^div.modal|^\.dropdown-backdrop)/i;
					this.config.stylesheetParser_validSelectors = /^(\w|\-)*?(\.|#)(\w|\-)+$/; 
					
					var styleSheets = ".$this->_getTemplateCSS()."
		...
		$document->addScriptDeclaration("
			CKEDITOR.tools.addHashFunction(function(div)
			{
				...
				{ 
					 ...
					 stylesheets : ".json_encode($excludes).",		
					 ...
					 magicline_enabled : ".(int) $magicline_enabled.",
		...
		$editor = JLayoutHelper::render('joomla.arkeditor.textarea', $textarea) .
	...
	private function _getTemplateCSS()
	{
		...
		if($this->app->isAdmin())
		{
			...
			$dom->recover = true;
			@$dom->loadHTMLFile(JURI::root());
		
			$links = $dom->getElementsByTagName('link');
			
			
			$excludes = $this->params->def('stylesheets',array('model','bootstrap','frontediting'));
