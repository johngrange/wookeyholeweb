...
abstract class AutotweetBaseHelper
{
	...
	static public function applyTextPattern($pattern, &$post)
	{
		...
		$pattern = self::_applyTextPattern($pattern, 'introtext', $post->message);
		$pattern = self::_applyTextPattern($pattern, 'text', $post->message);
	...
	public static function getItemid($comp_name, $needles, $q_view = 'view')
	{
		...
		// $menus = JApplication::getMenu('site', array()); - Strict Notice
