...
class SelectControlHelper
{
		...
		if (count($items))
		{
			foreach ($items as $item)
			{
				if ( (!AUTOTWEETNG_JOOCIAL)
					&& ($item->id >= AutotweetModelChanneltypes::TYPE_SCOOPIT_CHANNEL)
					&& ($item->id <= AutotweetModelChanneltypes::TYPE_EASYSOCIAL_CHANNEL) )
				{
					continue;
				}
	 ...
	 * @return  string  HTML
	 */
	public static function tumblrBlogs($blogs, $name, $attribs = array(), $selected = null, $idTag = null)
	{
