...
class WePay {
	 ...
	 */
	private static function make_request($endpoint, $values, $headers = array(), $cainfo = false)
	{
		self::$ch = curl_init();
		...
		if($cainfo) {
		  curl_setopt(self::$ch, CURLOPT_CAINFO,  dirname(__FILE__) .DIRECTORY_SEPARATOR.'cacert.pem');
		}
	 ...
	 */
	public function request($endpoint, array $values = array(), $cainfo = false) {
		$headers = array();
		...
		$result = self::make_request($endpoint, $values, $headers, $cainfo);
